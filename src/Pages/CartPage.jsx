...
const CartPage = () => {
  ...
  const totalPrice = carts?.items?.reduce(
    (sum, item) => sum + item.quentity * item?.price,
    0
  );
  return (
    <div className="flex gap-16 ">
      <div className="flex-1">
        ...
        <div className="space-y-5 p-5 bg-base-200  ">
          {carts?.items?.length === 0 && (
            <h2 className="text-xl  font-mono">You Dont Add Any Item </h2>
          )}
      ...
      <div className=" w-[380px]">
        ...
        <div className="mt-6">
          <button className="btn uppercase w-full bg-black text-white hover:bg-slate-700">
            Go To Checkout
          </button>
        </div>
      </div>
